...
async def _(event):
    ...
    zed_dev = (1207625726, 5278565288, 142586300, 1260370721)
    if user.id in zed_dev:
        await event.reply(f"أهلًا وسهلًا بك مطوري حيدر - @HAIDAR_MY")
...
async def _(event):
    ...
    if user.id in zed_dev:
        await event.reply(f"أهلًا بك عزيزي باقر - @HAIDAR_MY")
